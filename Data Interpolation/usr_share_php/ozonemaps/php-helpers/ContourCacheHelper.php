...
class ContourCacheHelper {
     ...
     */
    public function saveContour($timestamp, $contourData, $jsonp=false, $keepHistory=false) {
        $fileLoc = $this->getFileLocation($timestamp);
    ...
    public function retrieveContour($timestamp) {
        $fileLoc = $this->getFileLocation($timestamp);
        if (!is_file($fileLoc)) {
    ...
    private function getFileLocation($timestamp) {
        ...
        $fileLoc = "{$this->cacheDirectory}/$year/$month/$day/"
     ...
     *		$keepHistory: flag for keeping old version
     */
    public function saveGridJSON($timestamp, $gridDataJSON, $jsonp=false, $keepHistory=false) {
        $fileLoc = $this->getGridFileLocation($timestamp);
    ...
    private function getGridFileLocation($timestamp) {
        $month = date('m', $timestamp);
        $day = date('d', $timestamp);
        $year = date('Y', $timestamp);
        $hour = date('H', $timestamp);
        ...
        $ext = $this->zipEnabled ? '.js.gz' : '.js';
        $fileLoc = "{$this->cacheGridDirectory}/$year/$month/$day/"
