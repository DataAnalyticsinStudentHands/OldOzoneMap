...
push_script=/mnt/ibreathe/TCEQ/scripts/push_ibh_data5.pl
...
do
  ...
  if [ -d ${top_dir}/${mydate} ]; then
    ...
    if [ -n "${tmp_list}" ]; then
      ...
      #[[ "$debug" == "yes" ]] && echo "# executing: 'perl -g ${push_script} -d ${mylist}'" | sed 's/\/mnt\/ibreathe\/TCEQ\/data_extractor\/daily\/[0-9]\{4\}\/[0-9]\{4\}-[0-9]\{2\}\/[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}\///g'
      if [[ "$debug" == "yes" ]]; then
        echo "# executing: 'perl ${push_script} -v -g -d ${mylist}'" | sed 's/\/mnt\/ibreathe\/TCEQ\/data_extractor\/daily\/[0-9]\{4\}\/[0-9]\{4\}-[0-9]\{2\}\/[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}\///g'
        perl ${push_script} -v -g -d ${mylist}
      else
        perl ${push_script} -g -d ${mylist}
      fi
    else
