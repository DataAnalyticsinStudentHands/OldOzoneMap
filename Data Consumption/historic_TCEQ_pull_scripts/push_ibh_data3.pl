...
getopts("DdFhNo:s:STt:v", \%options);
...
#  PARTITION m$m VALUES LESS THAN (".timegm(0,0,0,1,$m,$year).'),';
	  $sql .= "
  PARTITION m$m VALUES LESS THAN (unix_timestamp('$year-".($m*2)."-1')),";
	} # for
	$sql .= "
...
#  PARTITION m$m VALUES LESS THAN (".timegm(0,0,0,1,$m,$year).'),';
	  $sql2 .= "
  PARTITION m$m VALUES LESS THAN (unix_timestamp('$year-".($m*3)."-1')),";
	} # for
	$sql2 .= "
	...
	if (length($flag) > 0) {
	  ...
	  if ($$my_ref{'epoch'} > 0) {
		...
		if (!exists($options{N})) {
	          ...
	          print $FH "# debug:   insert new transaction for epoch=$my_epoch and site=$site_list[$w]: $inst_type='$value' and $inst_type"."_flag='$flag' (was '$old_flag')\n" if exists($options{v}) && $more_debug;
	          #$sql_hash{'txn_new'} = "INSERT INTO $txn_tbl_name (epoch, siteID, inst, state, update_epoch) VALUES (?, ?, ?, ?, ?);";
		...
		$counter{'total_new'} ++;
	        ...
	        print $FH "# debug:   update transcation for epoch=$my_epoch and site=$site_list[$w]: $inst_type=$$my_ref{$inst_type} and $inst_type"."_flag='$$my_ref{$inst_type.'_flag'}' (was '$old_value' with '$old_flag')\@ ".epoch_to_datetime($download_epoch)." ($download_epoch)\n" if exists($options{v}) && $more_debug;
                #$sql_hash{'txn_change'} = "INSERT INTO $txn_tbl_name (epoch, siteID, inst, state, old_value, old_flag, update_epoch) VALUES (?, ?, ?, ?, ?, ?, ?);";
...
if ($counter{'total_row'} > 0 || $counter{'total_file'} != $counter{'total_file_seen'} || exists($options{v})) {
  ...
  print $FH '['.epoch_to_datetime(time()).'] Total: '.
	"$path_str : ".
