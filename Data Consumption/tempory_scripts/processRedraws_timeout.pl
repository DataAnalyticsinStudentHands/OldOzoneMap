...
$sqlSelect = "SELECT epoch FROM (SELECT DISTINCT epoch FROM ibh_data_year_2014 WHERE redraw = 1 AND o3 IS NOT NULL) AS t1 WHERE t1.epoch NOT IN (SELECT epoch FROM aggregate_o3_epoch) ORDER BY epoch DESC";
